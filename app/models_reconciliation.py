...
class ReconciliationIssue(Base):
    ...
    # Relationships
    run_id = Column(String(50), ForeignKey("reconciliation_runs.id"), nullable=False, index=True)
    run = relationship("ReconciliationRun")
    
    tenant_id = Column(String(50), ForeignKey("tenant.id"), nullable=False, index=True)
    tenant = relationship("Tenant")
    
    subscription_id = Column(String(50), ForeignKey("subscription.id"), nullable=True, index=True)
    subscription = relationship("Subscription")
    
    # Issue details
    issue_type = Column(
        SQLEnum(ReconciliationType),
        nullable=False,
        index=True
    )
    ...
    # Stripe details
    stripe_object_id = Column(String(255), nullable=True)  # subscription ID, customer ID, etc
    stripe_object_type = Column(String(50), nullable=True)  # subscription, customer, etc
    ...
    resolution_action = Column(String(500), nullable=True)  # What was done to fix it
...
class ManualReconciliationRequest(BaseModel):
    ...
    tenant_id: Optional[str] = Field(None, description="Specific tenant to reconcile")
    resolve_issues: bool = Field(False, description="Auto-resolve mismatches if possible")
